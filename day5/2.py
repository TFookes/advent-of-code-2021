...
if __name__ == "__main__":
    ...
    grid = np.zeros((max(max(x0), max(x1)) + 1, max(max(y0), max(y1)) + 1))

    for line in zip(x0, y0, x1, y1):
        if not (line[0] == line[2] or line[1] == line[3]):
            if line[0] < line[2] and line[1] < line[3]:
                for i, j in enumerate(range(line[0], line[2] + 1)):
                    grid[j, min(line[1], line[3]) + i] += 1
            elif line[0] < line[2] and line[1] > line[3]:
                for i, j in enumerate(range(line[0], line[2] + 1)):
                    grid[j, max(line[1], line[3]) - i] += 1
            elif line[0] > line[2] and line[1] < line[3]:
                for i, j in enumerate(range(line[2], line[0] + 1)):
                    grid[line[0] - i, min(line[1], line[3]) + i] += 1
            elif line[0] > line[2] and line[1] > line[3]:
                for i, j in enumerate(range(line[2], line[0] + 1)):
                    grid[line[0] - i, max(line[1], line[3]) - i] += 1
